...
/**
 * CursorOAuth - Cursor OAuth 认证实现
 * <p>
 * 实现 Cursor AI 编辑器的 OAuth 2.0 Authorization Code flow with PKCE。
 ...
 * <p>
 * Cursor 使用 auth0 作为身份认证提供商，遵循标准 OAuth 2.0 协议。
 */
public class CursorOAuth extends OAuthProvider {

    private static final String TAG = "CursorOAuth";

    // === OAuth 端点常量 ===
    private static final String AUTH_URL = "https://authenticate.cursor.sh/authorize";
    private static final String TOKEN_URL = "https://authenticate.cursor.sh/oauth/token";
    private static final String CLIENT_ID = "cursor_android_client";
    private static final String REDIRECT_URI = "http://localhost:1457/auth/callback";
    private static final int DEFAULT_CALLBACK_PORT = 1457;
    ...
    /**
     * 构造 CursorOAuth 实例，使用指定的代理 URL
     *
     * @param proxyUrl 代理 URL，为空则不使用代理
     */
    public CursorOAuth(String proxyUrl) {
    ...
    public OAuthFlow startAuth() throws OAuthException {
        ...
        return new OAuthFlow(authUrl, state, "cursor", future);
    }
    ...
    private String buildAuthorizationUrl(String state, PKCECodes pkceCodes) {
        ...
        sb.append("&audience=").append(encodeParam("https://api.cursor.com"));
        return sb.toString();
    }
    ...
    public static class JWTClaims {
        ...
        JWTClaims(String json) throws org.json.JSONException {
            ...
            // 解析 Cursor 自定义 claims
            // Cursor 使用 Auth0，通常将用户 ID 放在 sub 字段
            this.accountId = obj.optString("cursor_account_id", "");
            if (accountId.isEmpty()) {
                // 尝试从 Auth0 命名空间 claims 中提取
                JSONObject cursorAuth = obj.optJSONObject("https://cursor.sh/auth");
                if (cursorAuth != null) {
                    this.accountId = cursorAuth.optString("account_id", "");
                }
            }
            if (accountId.isEmpty()) {
    ...
    public static class CallbackServer {
        ...
        private String buildSuccessPage() {
            return "<!DOCTYPE html>" +
                    ...
                    "<title>Authentication Successful - Cursor</title>" +
                    "<style>" +
                    ...
                    "<p>You have successfully authenticated with Cursor. " +
        ...
        private String buildErrorPage(String error) {
            return "<!DOCTYPE html>" +
                    "<html lang=\"en\">" +
                    "<head><meta charset=\"UTF-8\">" +
                    "<title>Authentication Failed - Cursor</title>" +
                    "<style>" +
                    "body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;" +
                    "display:flex;justify-content:center;align-items:center;min-height:100vh;margin:0;" +
