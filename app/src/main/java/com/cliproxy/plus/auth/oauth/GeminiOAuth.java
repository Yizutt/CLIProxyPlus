...
/**
 * GeminiOAuth - Google Gemini/AI Studio OAuth 认证实现
 * <p>
 * 使用 Google OAuth 2.0 Authorization Code flow（PKCE）进行用户认证，
 * 支持 Gemini API 和 Google AI Studio 的令牌获取与刷新。
 * <p>
 * OAuth 流程：
...
public class GeminiOAuth extends OAuthProvider {
    ...
    /** Gemini API 客户端 ID（AI Studio 项目） */
    private static final String CLIENT_ID = "46748251226-1i1i1i1i1i1i1i1i1i1i1i1i1i1i1i.apps.googleusercontent.com";

    /** 本地回调 URI */
    private static final String REDIRECT_URI = "http://localhost:1477/auth/callback";

    /** 本地回调服务器端口 */
    private static final int DEFAULT_CALLBACK_PORT = 1477;

    /** 回调等待超时（秒） */
    private static final int CALLBACK_TIMEOUT_SECONDS = 300;

    /** Gemini API 作用域 */
    private static final String GEMINI_SCOPE = "openid email profile https://www.googleapis.com/auth/generative-language";
    ...
    /**
     * 构造 GeminiOAuth 实例，使用指定的代理 URL。
     *
     * @param proxyUrl 可选代理 URL，为空时不使用代理
     */
    public GeminiOAuth(String proxyUrl) {
    ...
    public AuthResult startAuth() throws OAuthException {
        ...
        try {
            ...
            log("Gemini OAuth: waiting for callback on port " + DEFAULT_CALLBACK_PORT);
            ...
            if (callback.error != null) {
                throw new OAuthException("callback_error",
                        "Google OAuth callback error: " + callback.error);
            }
            ...
            logError("Gemini OAuth callback failed", e);
    ...
    private String buildAuthorizationUrl(String state, PKCECodes pkceCodes) {
        ...
        sb.append("&scope=").append(encodeParam(GEMINI_SCOPE));
        ...
        try {
            ...
            if (!error.isEmpty()) {
                ...
                throw new OAuthException("token_error",
                        "Google OAuth error: " + error + " - " + errorDesc);
            }
    ...
    /**
     * 轻量级本地 HTTP 服务器，用于接收 Google OAuth 回调。
     * <p>
    ...
    public static class CallbackServer {
        ...
        private String buildSuccessPage() {
            return "<!DOCTYPE html>" +
                    ...
                    "<title>Authentication Successful - Gemini</title>" +
                    "<style>" +
                    ...
                    "<p>You have successfully authenticated with Google Gemini.<br>" +
        ...
        private String buildErrorPage(String error) {
            return "<!DOCTYPE html>" +
                    "<html lang=\"en\">" +
                    "<head><meta charset=\"UTF-8\">" +
                    "<title>Authentication Failed - Gemini</title>" +
                    "<style>" +
                    "body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;" +
                    "display:flex;justify-content:center;align-items:center;min-height:100vh;margin:0;" +
